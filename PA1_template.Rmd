...
output: 
 author: "bkotsane - PA1_template.Rmd"
  html_document:
    keep_md: true
---
...
```{r, message=FALSE}fileName <- "https://d396qusza40orc.cloudfront.net/repdata%2Fdata%2Factivity.zip"
...
```{r}stepsPerDay <- tapply(act_data$steps, act_data$date,sum)```

####mean and medium Number of Steps Taken Per Day
```{r}mean(stepsPerDay,na.rm = TRUE)```
```{r}`median(stepsPerDay,na.rm = TRUE)```

###Histogram of total number of steps taken per day
```{r}hist(stepsPerDay, xlab="number of steps per day", main="Histogram of Total Steps taken per day", col="light blue")```

## What is the average daily activity pattern?
```{r}stepsIntervals <- aggregate(act_data$steps, by=list(act_data$interval), mean, na.rm=TRUE, na.action=NULL)
...
```{r}act_data_miss <- merge(act_data, stepsIntervals, by = "interval", sort= FALSE)
